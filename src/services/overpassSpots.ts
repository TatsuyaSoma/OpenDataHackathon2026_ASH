...
// 都心部はコンビニ・自販機の密度が非常に高く、全件描画すると地図が埋め尽くされ描画も重くなるため、
// 種別ごとに件数の上限を設ける
const MAX_SPOTS_PER_TYPE = 40;

/**
 * 指定範囲内のコンビニ・自販機をOverpass APIから取得し、MapSpot形式に変換する。
 */
export const fetchConvenienceAndVendingSpots = async (bounds: OverpassBounds): Promise<MapSpot[]> => {
  ...
  const spots = data.elements
    .map((element): MapSpot | null => {
      ...
      if (!isConvenience && !isVending) return null;

      return {
        id: `osm-${element.id}`,
        type: isConvenience ? 'convenience' : 'vending',
        name: element.tags?.name ?? element.tags?.brand ?? (isConvenience ? 'コンビニ' : '自販機'),
